...
export const Voucher = () => {
	...
	const col = [
		...
		},
		{
			...
			key: 'status', 
			fixed: 'left',
			render: (text, record, index) => <Tag color={record.status === 'available' ? "#87d068" : "#f50"}>{record.status}</Tag>
		},
		{
			title: 'Discount (%)',
			dataIndex: 'discount',
		...
		},
		{
			...
			render: (text, record, index) => <span>
				{record.discountLimit.toLocaleString("it-IT", {
					style: "currency",
					currency: "VND",
				})}{" "}
			</span>
		...
		},
		{
			...
			align: 'center',
			key: 'used', 
			render: (text, record, index) => record.amount - record.remainingAmount
		},
		{
			title: 'Start Date',
			dataIndex: 'startDate',
